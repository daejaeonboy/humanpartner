...
const DEFAULT_NOTICE_CATEGORY = '일반';
...
export const NoticeManager: React.FC = () => {
  ...
  const getCategoryOptions = (itemList = items, categoryList = categories) => {
    const names = getNoticeCategoryNames(categoryList, itemList);
    return names.length > 0 ? names : [DEFAULT_NOTICE_CATEGORY];
  };
  ...
  const loadCategories = async () => {
    const data = await getAllNoticeCategories();
    setCategories(data);
    return data;
  };
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        console.error('Failed to load notice manager data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const openAddModal = () => {
    ...
    setFormData({
      category: categoryOptions[0] || DEFAULT_NOTICE_CATEGORY,
      title: '',
      summary: '',
      content: '',
      image_url: '',
  ...
  const openEditModal = (item: Notice) => {
    ...
    setFormData({
      ...item,
      category: normalizeContentCategoryName(item.category) || categoryOptions[0] || DEFAULT_NOTICE_CATEGORY,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        category: normalizeContentCategoryName(formData.category) || DEFAULT_NOTICE_CATEGORY,
      ...
      if (editingItem?.id) {
        ...
        await addNotice(payload as Omit<Notice, 'id' | 'created_at' | 'updated_at'>);
      }
      ...
      alert('공지사항 저장에 실패했습니다. 카테고리 SQL 테이블 생성 여부를 확인해주세요.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const toggleActive = async (item: Notice) => {
    ...
    try {
      await updateNotice(item.id, { is_active: !item.is_active });
      await loadItems();
    } catch (error) {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      console.error('Failed to add notice category:', error);
  ...
  const handleUpdateCategory = async (id: string) => {
    ...
    setCategoryLoading(true);
    try {
      await updateNoticeCategory(id, { name: nextName });
      ...
      await Promise.all(
        affectedItems
          .filter((item) => item.id)
          .map((item) => updateNotice(item.id!, { category: nextName }))
      );
      ...
      console.error('Failed to update notice category:', error);
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    if (usedCount > 0) {
      alert(`이 카테고리를 사용 중인 공지사항이 ${usedCount}개 있습니다. 먼저 게시물의 카테고리를 변경해주세요.`);
      return;
    }
    ...
    try {
      ...
      console.error('Failed to delete notice category:', error);
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-800">
            <Megaphone size={24} className="text-[#39B54A]" />
            공지사항 관리
          </h1>
          <p className="mt-1 text-sm text-slate-500">공지사항 페이지에 노출할 게시물과 카테고리를 관리합니다.</p>
        </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border-2 border-dashed border-slate-200 bg-white p-20 text-center">
          ...
          <p className="text-lg font-medium text-slate-500">등록된 공지사항이 없습니다.</p>
          <p className="mt-1 text-sm text-slate-400">공지사항 추가 버튼으로 첫 게시물을 등록해보세요.</p>
        </div>
      ) : (
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider">
                  ...
                  <span className="rounded-md bg-[#39B54A]/10 px-2 py-0.5 text-[#39B54A]">
                    {normalizeContentCategoryName(item.category) || DEFAULT_NOTICE_CATEGORY}
                  </span>
                  <span
          ...
          <div className="flex min-h-full items-start justify-center py-4">
            <div className="flex w-full max-w-3xl max-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-[28px] bg-white shadow-2xl">
              <div className="flex shrink-0 items-center justify-between border-b border-slate-100 p-6">
                <h2 className="text-xl font-bold text-slate-900">{editingItem ? '공지사항 수정' : '새 공지사항 등록'}</h2>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-5 overflow-y-auto p-8">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <select
                      value={normalizeContentCategoryName(formData.category) || categoryOptions[0] || DEFAULT_NOTICE_CATEGORY}
          ...
          <div className="bg-white rounded-[32px] w-full max-w-md overflow-hidden shadow-2xl animate-fadeIn">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {categories.length === 0 ? (
                  <div className="text-center py-8 text-slate-400 text-sm">
                    ...
                    <p className="text-xs mt-3 text-slate-300">공지사항 카테고리 SQL을 먼저 실행해야 합니다.</p>
                  </div>
                ) : (
                  categories
                          ...
                          {editingCategoryId === category.id ? (
                            ...
                            <>
                              <div className="flex-1 min-w-0">
                                ...
                                <span className="text-xs text-slate-400">사용 중 공지사항 {usedCount}건</span>
                              </div>
              ...
              <p className="text-xs text-slate-400 text-center pt-2">
                카테고리명을 수정하면 해당 카테고리의 공지사항도 자동으로 업데이트됩니다.
              </p>
            </div>
          </div>
        </div>
