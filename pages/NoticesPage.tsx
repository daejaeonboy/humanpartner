...
import {
  getNoticeCategoryEntries,
  getNoticeCategoryNames,
  getNoticeCategoryTableItems,
  getNoticesPage,
  Notice,
  NoticeCategory,
...
export const NoticesPage: React.FC = () => {
  ...
  const [categories, setCategories] = React.useState<NoticeCategory[]>([]);
    ...
    const fetchCategories = async () => {
      try {
        const [categoryData, entryData] = await Promise.all([
          getNoticeCategoryTableItems().catch((error) => {
            console.error('Failed to load managed notice categories:', error);
            return [];
          }),
          getNoticeCategoryEntries().catch((error) => {
            console.error('Failed to load notice category entries:', error);
            return [];
          }),
        ]);
  ...
  React.useEffect(() => {
    ...
    const fetchItems = async () => {
      ...
      try {
        const { data, count } = await getNoticesPage({
          page: currentPage,
  ...
  const categoryTabs = ['전체', ...getNoticeCategoryNames(categories, categoryEntries)];
  ...
  const emptyMessage = hasKeyword ? '검색 결과가 없습니다.' : '등록된 공지사항이 없습니다.';

  return (
    <>
      <Seo
        title="공지사항 | 행사어때"
        description="행사어때 공지사항을 확인할 수 있습니다."
        canonical="/notices"
      />
      ...
      <div className="bg-white min-h-screen pb-20">
        <Container>
          ...
          {loading || listLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-10 mb-16">
              {items.map((item) => (
                <article key={item.id} className="group">
                  <Link to={`/notices/${item.id}`} className="block">
